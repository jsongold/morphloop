...
- 起動・停止・ログ・pack import・e2e は必ず `scripts/dev-stack.sh` を使う。
- このスクリプトは worktree パスから compose プロジェクト名 `morphloop-<hash>` とホストポート（api 18000+ / web 13000+）を導出し、その worktree 専用のスタックを立てる。
- 主な使い方:
  - `./scripts/dev-stack.sh up`       # build + 起動 + pack import
  - `./scripts/dev-stack.sh import`   # pack を import（既定 contents/software-engineering）
  - `./scripts/dev-stack.sh logs [api|web|db]`
